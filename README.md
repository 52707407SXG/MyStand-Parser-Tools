...
HTTP 服务一旦启用 token、`--require-token` 或公开绑定，就会进入本地文件安全模式：只有 `MYSTAND_PARSER_ALLOWED_ROOTS` 内的文件可以读取；未配置白名单时，HTTP 服务会拒绝本地文件解析，只允许后续改为 URL/对象存储/上传目录方案。普通本地 CLI 仍保持兼容，不强制白名单。
...
- 限制最大文件大小。
- HTTP/token 模式限制本地文件读取目录：`MYSTAND_PARSER_ALLOWED_ROOTS`。
- 限制普通 URL 响应体大小：`MYSTAND_PARSER_MAX_URL_BYTES`。
- 限制 ZIP 解压后总大小和文件数量。
- 拦截 ZIP 路径穿越。
- URL 只允许 `http` / `https`。
- 拦截 localhost、IPv4/IPv6 内网 IP、IPv6 mapped localhost、十进制/八进制写法的本地 IP、`.local`、`.internal`、`.lan`，包括 DNS 解析到内网的情况。
