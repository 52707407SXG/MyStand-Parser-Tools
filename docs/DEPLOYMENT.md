...
`--max-jobs` / `MYSTAND_PARSER_MAX_JOBS` 只限制 `pending/running` 活跃任务。已经完成的 `done/failed` 记录不会占用新任务名额，历史记录由 `--max-job-history` / `MYSTAND_PARSER_MAX_JOB_HISTORY` 和 `--job-history-ttl` / `MYSTAND_PARSER_JOB_HISTORY_TTL_SECONDS` 控制。
